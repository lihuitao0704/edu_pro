...
当前验收数据库账号统一密码：`Demo@123`（仅限本地演示环境）。

| 角色 | 当前账号范围 | 建议验收账号 | 主要页面 |
| --- | --- | --- | --- |
| 普通客户 | `user_0001`～`user_0100` | `user_0001` | 智能对话、客户画像 |
| 理财顾问 | `emp_001`～`emp_006` | `emp_001` | 顾问工作台、智能投顾、数据分析 |
| 风控专员 | `emp_007`～`emp_011` | `emp_007` | 风险预警、工单处理 |
| 客户经理 | `emp_012`～`emp_016` | `emp_012` | 客户工作台、业务办理与审批 |
| 管理员 | `emp_017`～`emp_020` | `emp_017` | 全部管理功能、知识库 |

> `demo_admin`、`demo_customer_01` 等旧种子账号不属于当前验收数据库。
> 正式环境必须关闭 `AUTH_MOCK_MODE`，并为每个账号设置独立密码。
