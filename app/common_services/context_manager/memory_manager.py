...
class MemoryManager:
    """Shared owner-scoped session memory facade backed by Redis.
    ...
    Session context persists across service restarts via Redis-backed
    SessionContextStore (with in-memory fallback).
    """
    ...
    async def load_context(self, session_id: str, actor_id: int) -> dict:
        cached = await self.session_store.get(session_id, actor_id)
        ...
        if persisted is None or int(persisted.user_id) != actor_id:
            return {}
        return await self.session_store.update(
    ...
    async def save_context(self, session_id: str, actor_id: int, entities: dict, **values: object) -> dict:
        return await self.session_store.update(session_id, actor_id, entities, **values)
